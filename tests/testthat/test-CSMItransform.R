test_that("csmi 2010 loads", {
  hasExpectedNames <- function(df) {
    c(
      "Stn Depth (m)", "Notes", "STIS#", "FRACTION", "LATITUDE", "LONGITUDE", "ANALYTE", "UNITS",
      "RESULT", "mdl", "Date"
    ) %in% names(df)
  }
...
test_that("csmi 2015 loads", {
  hasExpectedNames <- function(df) {
    c(
      "STIS#", "SAMPLE_DEPTH", "ANALYTE", "RESULT",
      "UNITS", "mdl", "AnalMethod", "ANL_CODE",
      "LATITUDE", "LONGITUDE", "Date", "STATION_DEPTH"
    ) %in% names(df)
  }
...
test_that("csmi 2021 loads", {
  hasExpectedNames <- function(df) {
    c(
      "SITE_ID", "Date", "Latitude", "Longitude", "SAMPLE_DEPTH",
      "ANALYTE", "UNITS", "RESULT", "mdl", "QA_CODE"
    ) %in% names(df)
  }
...
test_that("csmi loads", {
  hasExpectedNames <- function(df) {
    c(
      "Stn Depth (m)", "STIS#", "FRACTION", "LATITUDE",

...
df <- read_csv(paste0(dataDir, "/Raw_Data/allWQ.csv"), na = c("", "NA", "no result reported", "No result reported.", "No result recorded.", "No Result Reported", "No Result Reported.", "NO RESULT", "no result", "No result reported",
...
lakes <- read_sf(paste0(dataDir, "/Other/ne_10m_lakes.shp"))
...
df %>%
  filter(ANALYTE == "Turbidity", VALUE < 60) %>%
  ggplot(aes(fill = UNITS, x = VALUE)) + 
...
remarkCodes <- read_csv(paste0(dataDir, "/Raw_Data/remarkCodes.csv"))
QCCodes <- read_csv(paste0(dataDir, "/Raw_Data/QCcodes.csv"))
depthCodes <- read_csv(paste0(dataDir, "/Raw_Data/DepthCodes.csv"))
...
df %>%
  filter(grepl("Other", RESULT_REMARK))
```

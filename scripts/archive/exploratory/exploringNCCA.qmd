...
```{r loadNSetup}
# make a conversions table for units
# Add year to NCCA naming
# Add method to NCCA naming
# revisit the bimodality test
#   stratified by depth and season
...
library(tidyverse)
library(kableExtra)
qaCodes <- readxl::read_xlsx("Data/Meta/DRAFT_NCCA_QA_Codes_05.30.2017.xlsx", sheet = 1) %>%
  select(-...3) %>%
...
df <- readNCCA(siteFiles = siteFiles, preFiles = NULL, tenFiles = tenFiles, fifteenFiles = fifteenFiles)
...
# Generate summary of unique analytes
df %>%
  mutate(Year = year(Date)) %>%
  reframe(
    minY = min(Year, na.rm = T), maxY = max(Year, na.rm = T),
    Units = toString(unique(UNITS)),
    .by = c(ANALYTE, ANL_CODE, METHOD)
  ) %>%
  unite(Years, minY, maxY, sep = "-") %>%
  arrange(ANALYTE) %>%
  write_csv("temp.csv")
```
...
ggplot(lakes) +
  ...
  theme(
    panel.background = element_rect(fill = "#d0d890"),

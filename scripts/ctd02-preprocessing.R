...
# [ ] KV: Not a high priority, but note that the CTD file paths here are just synced files from Sharepoint and could probably be read in directly from Sharepoint, rather than having a user-specific path here. These paths can be found if you go to the files on Teams and hit 'Copy link'
  ...
  mutate(
    data = purrr::map(ctdFiles, \(x)
      ...
      .oce2df(suppressWarnings(oce::read.oce(x)), studyName = "NOAActd", bin = TRUE, downcast = TRUE) %>%
        # Note: The above .oce2df() function is from ctd-functions.R

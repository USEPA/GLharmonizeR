...
devtools::load_all()
#start.time <- Sys.time()
lakeMichigan <- assembleData(.test = F, out = "data/lakeMichigan", binaryOut = TRUE)
# assembly takes 1 minute
# end.time <- Sys.time()
# time.taken <- end.time - start.time
# time.taken
#df <- readRDS("fullData.Rds")
# processedDf <- imputeNwiden(df, latlonDigits = 4, imputeMethod = "uniform", timeBinning = "days", depthCuts = c(-0.1, 5, 15, 25, 50, max(df$sampleDepth, na.rm=T)), 
#   startDate= "2000-01-01", endDate= "2050-01-01")
# saveRDS(processedDf, "processedData.Rds")
```
...
df %>%
  reframe(
    ...
    across(c(CodeName, Latitude, RESULT, LongName, Units, MDL), function(x) mean(is.na(x))),
    .by = Study)  %>%
    arrange(Study)
    saveRDS(stringr::str_glue("tests/testthat/fixtures/dataMissingness{Sys.Date()}.Rds"))
readRDS(stringr::str_glue("tests/testthat/fixtures/dataMissingness{Sys.Date()}.Rds")) %>% View()

df %>%
  mutate(RESULT = ifelse(is.na(RESULT), mean(MDL, na.rm = T), RESULT), .by = CodeName) %>%
  reframe(m = mean(is.na(RESULT)), .by = c(Study, CodeName)) %>%
  arrange(desc(m))

df %>%
  reframe(
    # mean and SD of numeric columns
    across(c(Latitude, Longitude, RESULT), list(mean, sd), .names ="{.col}.{.fn}", na.rm = T),
    .by = c(Study, CodeName)) %>%
    arrange(Study, CodeName) %>%  #View()
...
write_csv(rounded3, "3digitStationOverlap.csv") # this matches the NCCA protocol resolution 
...
# Duplicates and pivoting
```{r duplicates}

removedDup <- df %>%  # 185374 partial observations
  drop_na(sampleDateTime, Latitude, CodeName, sampleDepth) %>%
  dplyr::mutate(
    Lat4 = round(Latitude, digits=4),
    Lon4 = round(Longitude, digits=4),
  ) %>%
  reframe(count = n(), Study = toString(unique(Study)), .by= c(Lat4, Lon4, sampleDateTime, CodeName, sampleDepth))

test <- removedDup %>%
  filter(count >1) %>%
  arrange(Lat4, Lon4, sampleDepth, sampleDateTime)
...
removedDup <- df %>%  # removes 10% 
  drop_na(sampleDateTime, Latitude, CodeName, sampleDepth) %>%
  dplyr::mutate(
    Lat = round(Latitude, digits=3),
    Lng = round(Longitude, digits=3),
  ) %>%
  reframe(

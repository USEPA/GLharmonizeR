...
test2 <- test %>%
  mutate(
    ...
    DateArea2 = stringr::str_remove_all(DateArea2, "[:alpha:]"),
    DateArea3 = stringr::str_remove_all(DateArea3, "[:alpha:]"),

    # clean up date areas
    dplyr::across(dplyr::starts_with("DateArea"), function(x) stringr::str_remove_all(x, "^-|^\\_|-$|\\_$|\\.")),
    dplyr::across(dplyr::starts_with("DateArea"), function(x) stringr::str_remove_all(x, "^[:digit:]*_")),
    dplyr::across(dplyr::starts_with("DateArea"), function(x) stringr::str_remove_all(x, "_[:digit:]*$")),
    ...
    dplyr::across(dplyr::matches("Date1.2"), function(x) as.POSIXlt.Date(x, tryFormats = c("jy"))),
...
write_csv(test2, "noaaFileParsing.csv")
